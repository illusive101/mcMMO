...
import com.gmail.nossr50.skills.Scythes;
...
public class Combat {
		...
		case PLAYER:
			...
			if (ItemChecks.isSword(itemInHand)
				...
				if (PPa.getAbilityMode(AbilityType.SKULL_SPLIITER)
						&& mcPermissions.getInstance().skullSplitter(attacker)) {
					applyAbilityAoE(attacker, target, event.getDamage(),
							plugin, SkillType.AXES);
				}
				...
				startGainXp(attacker, PPa, target, SkillType.SCYTHES, plugin);
			} else if (itemInHand.getType().equals(Material.AIR)
					&& mcPermissions.getInstance().unarmed(attacker)) {
				if (mcPermissions.getInstance().unarmedBonus(attacker)) {
					Unarmed.unarmedBonus(PPa, event);
				}

				if (PPa.getAbilityMode(AbilityType.BERSERK)
						&& mcPermissions.getInstance().berserk(attacker)) {
					event.setDamage((int) (event.getDamage() * 1.5));
				}

				if (targetType.equals(EntityType.PLAYER)
						&& mcPermissions.getInstance().disarm(attacker)) {
				...
				startGainXp(attacker, PPa, target, SkillType.UNARMED, plugin);
					...
					&& mcPermissions.getInstance().beastLore(attacker)) {
	 ...
	 * 
	 * @param target
	 *            LivingEntity which to attempt to damage
	 * @param dmg
	 *            Amount of damage to attempt to do
	 */
	 ...
	 *            Amount of damage to attempt to do
	 * @param cause
	 *            DamageCause to pass to damage event
	 */
	private static void dealDamage(LivingEntity target, int dmg,
			DamageCause cause) {
		if (LoadProperties.eventCallback) {
			EntityDamageEvent ede = (EntityDamageEvent) new FakeEntityDamageEvent(
					target, cause, dmg);
	 ...
	 *            Amount of damage to attempt to do
	 * @param attacker
	 *            Player to pass to event as damager
	 */
	private static void dealDamage(LivingEntity target, int dmg, Player attacker) {
		if (LoadProperties.eventCallback) {
			EntityDamageEvent ede = (EntityDamageByEntityEvent) new FakeEntityDamageByEntityEvent(
					attacker, target,
	 ...
	 */
	private static void applyAbilityAoE(Player attacker, LivingEntity target,
			int damage, mcMMO plugin, SkillType type) {
		int numberOfTargets = m.getTier(attacker.getItemInHand()); // The higher
																	// the
																	// weapon
																	// tier, the
																	// more
		...
		for (Entity entity : target.getNearbyEntities(2.5, 2.5, 2.5)) {
			...
			if (entity instanceof LivingEntity && numberOfTargets >= 1) {
				if (entityType.equals(EntityType.PLAYER)) {
					...
					if (defender.getName().equals(attacker.getName())) { // Is
																			// this
																			// even
																			// possible?
						continue;
					...
					if (target.getWorld().getPVP()) {
						...
						if (type.equals(SkillType.AXES)) {
							...
							message = mcLocale.getString("Scythes.HitByPlague");
	 ...
	 *            mcMMO plugin instance
	 */
	public static void startGainXp(Player attacker, PlayerProfile PP,
			LivingEntity target, SkillType skillType, mcMMO pluginx) {
		double baseXP = 0;
		...
		if (target instanceof Player) {
			...
			if (System.currentTimeMillis() >= (PPd.getRespawnATS() * 1000) + 5000
					&& ((PPd.getLastLogin() + 5) * 1000) < System
							.currentTimeMillis() && defender.getHealth() >= 1) {
				baseXP = 20 * LoadProperties.pvpxprewardmodifier;
			}
		} else if (!target.hasMetadata("mcmmoFromMobSpawner")) {
			if (target instanceof Animals
					&& !target.hasMetadata("mcmmoSummoned")) {
		...
		if (baseXP != 0) {
			Bukkit.getScheduler().scheduleSyncDelayedTask(pluginx,
					new GainXp(attacker, PP, skillType, baseXP, target), 0);
		}
	}
}

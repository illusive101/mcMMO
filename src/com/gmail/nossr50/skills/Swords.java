...
public class Swords {
	...
	public static void counterAttackChecks(EntityDamageByEntityEvent event) {
		...
		if (target instanceof Player) {
			...
			if (ItemChecks.isSword(defender.getItemInHand())
					&& mcPermissions.getInstance().counterAttack(defender)) {
				final int MAX_BONUS_LEVEL = 600;
				final int COUNTER_ATTACK_MODIFIER = 4;

				int skillLevel = PPd.getSkillLevel(SkillType.SWORDS);
				int skillCheck = m.skillCheck(skillLevel, MAX_BONUS_LEVEL);

				if (random.nextInt(2000) <= skillCheck) {
					Combat.dealDamage((LivingEntity) attacker,
							event.getDamage() / COUNTER_ATTACK_MODIFIER);
					defender.sendMessage(mcLocale
							.getString("Swords.CounterAttacked"));

					if (attacker instanceof Player) {
						((Player) attacker).sendMessage(mcLocale
								.getString("Swords.HitByCounterAttack"));
					}
				}
			}
		}
	...
	public static void heroicStrike(Player attack, EntityDamageByEntityEvent event) {
		...
		if (target instanceof Player) {
			...
			if (random.nextInt(5000) <= skillCheck) {
				attack.addPotionEffect(new PotionEffect(
						PotionEffectType.REGENERATION, 100, 1));
				attack.sendMessage(mcLocale.getString("Swords.HeroicStrike"));
			}
		}
	}

	/* Disable of swords slowing
	public static void slowCheck(Player attack, EntityDamageByEntityEvent event) {
		...
		if (target instanceof Player) {
			...
			if (random.nextInt(5000) <= skillCheck) {
				...
				defender.sendMessage(mcLocale.getString("Swords.HitSlowEffect"));

				if (attack instanceof Player)
					attack.sendMessage(mcLocale.getString("Swords.SlowEffect"));
			}
		}
	}
	*/

...
public class PlayerProfile {
	...
	HashMap<SkillType, Integer> skills = new HashMap<SkillType, Integer>(); // Skills
																			// and
																			// Levels
	HashMap<SkillType, Integer> skillsXp = new HashMap<SkillType, Integer>(); // Skills
																				// and
																				// XP
	...
	public PlayerProfile(String name) {
		...
		if (LoadProperties.useMySQL) {
			if (!loadMySQL()) {
				addMySQLPlayer();
				loadMySQL(); // This is probably not needed anymore, could just
								// delete. // So can we remove this whole
								// function, or just this line?
			}
		} else if (!load()) {
			addPlayer();
		}
	...
	/*
	 * Why do we even have this? The only time it's called, it's false. Why not
	 * combine the two?
	 */
	...
	public boolean loadMySQL() {
		...
		id = mcMMO.database.getInt("SELECT id FROM "
				+ LoadProperties.MySQLtablePrefix + "users WHERE user = '"
				+ playerName + "'");
		if (id == 0)
			return false;
		this.userid = id;
		if (id > 0) {
			HashMap<Integer, ArrayList<String>> huds = mcMMO.database
					.read("SELECT hudtype FROM "
							+ LoadProperties.MySQLtablePrefix
							+ "huds WHERE user_id = " + id);
			if (huds.get(1) == null) {
				mcMMO.database.write("INSERT INTO "
						+ LoadProperties.MySQLtablePrefix
						+ "huds (user_id) VALUES (" + id + ")");
			} else {
			...
			HashMap<Integer, ArrayList<String>> users = mcMMO.database
					.read("SELECT lastlogin, party FROM "
							+ LoadProperties.MySQLtablePrefix
							+ "users WHERE id = " + id);
			...
			if (cooldowns.get(1) == null) {
				mcMMO.database.write("INSERT INTO "
						+ LoadProperties.MySQLtablePrefix
						+ "cooldowns (user_id) VALUES (" + id + ")");
			} else {
				skillsDATS.put(AbilityType.SUPER_BREAKER,
						Integer.valueOf(cooldowns.get(1).get(0)));
				...
				skillsDATS.put(AbilityType.SERRATED_STRIKES,
						Integer.valueOf(cooldowns.get(1).get(5)));
				skillsDATS.put(AbilityType.SKULL_SPLIITER,
						Integer.valueOf(cooldowns.get(1).get(6)));
				skillsDATS.put(AbilityType.BLAST_MINING,
						Integer.valueOf(cooldowns.get(1).get(7)));
				skillsDATS.put(AbilityType.GRIM_STRIKE,
						Integer.valueOf(cooldowns.get(1).get(8)));
			}
			HashMap<Integer, ArrayList<String>> stats = mcMMO.database
					.read("SELECT taming, mining, repair, woodcutting, unarmed, herbalism, excavation, archery, swords, axes, acrobatics, fishing, scythes FROM "
							+ LoadProperties.MySQLtablePrefix
			...
			skills.put(SkillType.WOODCUTTING,
					Integer.valueOf(stats.get(1).get(3)));
			skills.put(SkillType.UNARMED, Integer.valueOf(stats.get(1).get(4)));
			skills.put(SkillType.HERBALISM,
					Integer.valueOf(stats.get(1).get(5)));
			skills.put(SkillType.EXCAVATION,
					Integer.valueOf(stats.get(1).get(6)));
			...
			skills.put(SkillType.ACROBATICS,
					Integer.valueOf(stats.get(1).get(10)));
			skills.put(SkillType.FISHING, Integer.valueOf(stats.get(1).get(11)));
			skills.put(SkillType.SCYTHES, Integer.valueOf(stats.get(1).get(12)));
			HashMap<Integer, ArrayList<String>> experience = mcMMO.database
					.read("SELECT taming, mining, repair, woodcutting, unarmed, herbalism, excavation, archery, swords, axes, acrobatics, fishing, scythes FROM "
							+ LoadProperties.MySQLtablePrefix
							+ "experience WHERE user_id = " + id);
			skillsXp.put(SkillType.TAMING,
					Integer.valueOf(experience.get(1).get(0)));
			skillsXp.put(SkillType.MINING,
					Integer.valueOf(experience.get(1).get(1)));
			skillsXp.put(SkillType.REPAIR,
					Integer.valueOf(experience.get(1).get(2)));
			skillsXp.put(SkillType.WOODCUTTING,
					Integer.valueOf(experience.get(1).get(3)));
			skillsXp.put(SkillType.UNARMED,
					Integer.valueOf(experience.get(1).get(4)));
			skillsXp.put(SkillType.HERBALISM,
					Integer.valueOf(experience.get(1).get(5)));
			skillsXp.put(SkillType.EXCAVATION,
					Integer.valueOf(experience.get(1).get(6)));
			skillsXp.put(SkillType.ARCHERY,
					Integer.valueOf(experience.get(1).get(7)));
			skillsXp.put(SkillType.SWORDS,
					Integer.valueOf(experience.get(1).get(8)));
			skillsXp.put(SkillType.AXES,
					Integer.valueOf(experience.get(1).get(9)));
			skillsXp.put(SkillType.ACROBATICS,
					Integer.valueOf(experience.get(1).get(10)));
			skillsXp.put(SkillType.FISHING,
					Integer.valueOf(experience.get(1).get(11)));
			skillsXp.put(SkillType.SCYTHES,
					Integer.valueOf(experience.get(1).get(12)));
			loaded = true;
			return true;
		} else {
			return false;
	...
	public void addMySQLPlayer() {
		int id = 0;
		mcMMO.database.write("INSERT INTO " + LoadProperties.MySQLtablePrefix
				+ "users (user, lastlogin) VALUES ('" + playerName + "',"
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (character.length > 4 && m.isInt(character[4]))
					skillsXp.put(SkillType.MINING,
							Integer.valueOf(character[4]));
				if (character.length > 5 && m.isInt(character[5]))
					skills.put(SkillType.WOODCUTTING,
							Integer.valueOf(character[5]));
				...
				if (character.length > 9 && m.isInt(character[9]))
					skills.put(SkillType.HERBALISM,
							Integer.valueOf(character[9]));
				if (character.length > 10 && m.isInt(character[10]))
					skills.put(SkillType.EXCAVATION,
							Integer.valueOf(character[10]));
				if (character.length > 11 && m.isInt(character[11]))
					skills.put(SkillType.ARCHERY,
							Integer.valueOf(character[11]));
				...
				if (character.length > 31)
					skillsDATS.put(AbilityType.SKULL_SPLIITER,
							Integer.valueOf(character[31]));
				if (character.length > 32)
					skillsDATS.put(AbilityType.SUPER_BREAKER,
							Integer.valueOf(character[32]));
				...
				if (character.length > 34)
					skills.put(SkillType.FISHING,
							Integer.valueOf(character[34]));
				if (character.length > 35)
					skillsXp.put(SkillType.FISHING,
							Integer.valueOf(character[35]));
	...
	public void save() {
		Long timestamp = System.currentTimeMillis() / 1000; // Convert to
															// seconds
		// if we are using mysql save to database
		if (LoadProperties.useMySQL) {
			mcMMO.database.write("UPDATE " + LoadProperties.MySQLtablePrefix
					+ "huds SET " + " hudtype = '" + hud.toString()
					+ "' WHERE user_id = " + this.userid);
			mcMMO.database.write("UPDATE " + LoadProperties.MySQLtablePrefix
					+ "users SET lastlogin = " + timestamp.intValue()
					+ " WHERE id = " + this.userid);
			mcMMO.database.write("UPDATE " + LoadProperties.MySQLtablePrefix
					+ "users SET party = '" + this.party + "' WHERE id = "
					+ this.userid);
			mcMMO.database.write("UPDATE " + LoadProperties.MySQLtablePrefix
					+ "cooldowns SET " + " mining = "
					+ skillsDATS.get(AbilityType.SUPER_BREAKER) + " , grims = "
			...
			mcMMO.database.write("UPDATE " + LoadProperties.MySQLtablePrefix
					+ "skills SET " + "  taming = "
					+ skills.get(SkillType.TAMING) + ", mining = "
					+ skills.get(SkillType.MINING) + ", repair = "
					+ skills.get(SkillType.REPAIR) + ", woodcutting = "
					+ skills.get(SkillType.WOODCUTTING) + ", unarmed = "
					+ skills.get(SkillType.UNARMED) + ", herbalism = "
					+ skills.get(SkillType.HERBALISM) + ", excavation = "
					+ skills.get(SkillType.EXCAVATION) + ", archery = "
					+ skills.get(SkillType.ARCHERY) + ", swords = "
			...
			mcMMO.database.write("UPDATE " + LoadProperties.MySQLtablePrefix
					...
					+ skillsXp.get(SkillType.TAMING) + ", mining = "
					+ skillsXp.get(SkillType.MINING) + ", repair = "
					+ skillsXp.get(SkillType.REPAIR) + ", woodcutting = "
					+ skillsXp.get(SkillType.WOODCUTTING) + ", unarmed = "
					+ skillsXp.get(SkillType.UNARMED) + ", herbalism = "
					+ skillsXp.get(SkillType.HERBALISM) + ", excavation = "
					+ skillsXp.get(SkillType.EXCAVATION) + ", archery = "
					+ skillsXp.get(SkillType.ARCHERY) + ", swords = "
			...
			try {
				...
				while ((line = in.readLine()) != null) {
					...
					if (!line.split(":")[0].equalsIgnoreCase(playerName)) {
						...
						writer.append(String.valueOf(skillsDATS
								.get(AbilityType.GIGA_DRILL_BREAKER)) + ":");
						writer.append(String.valueOf(skillsDATS
						...
						writer.append(String.valueOf(skillsDATS
								.get(AbilityType.SKULL_SPLIITER)) + ":");
						writer.append(String.valueOf(skillsDATS
								.get(AbilityType.SUPER_BREAKER)) + ":");
	...
	public void resetAllData() {
		// This will reset everything to default values and then save the
		// information to FlatFile/MySQL
		for (SkillType skillType : SkillType.values()) {
			if (skillType != SkillType.ALL) {
				skills.put(skillType, 0);
				skillsXp.put(skillType, 0);
	 ...
	 * 
	 * @param tool
	 *            Tool to set the mode for
	 * @param bool
	 *            true if the tool should be prepped, false otherwise
	 */
	 ...
	 *            The ability to check
	 * @param bool
	 *            True if the ability is informed, false otherwise
	 */
		...
		if (skillType.equals(SkillType.ALL)) {
			for (SkillType x : SkillType.values()) {
				...
				Bukkit.getPluginManager().callEvent(
						new McMMOPlayerXpGainEvent(player, x, newValue));
				skillsXp.put(x, skillsXp.get(x) + newValue);
			}
		} else {
			Bukkit.getPluginManager().callEvent(
					new McMMOPlayerXpGainEvent(player, skillType, newValue));
	...
	public void addXP(SkillType skillType, int newValue) {
		...
		if (System.currentTimeMillis() < ((xpGainATS * 1000) + 250)
				|| player.getGameMode().equals(GameMode.CREATIVE)) {
			return;
		}

		xpGainATS = (int) (System.currentTimeMillis() / 1000); // Setup a
																// timestamp of
																// when xp was
																// given
		...
		int xp = (int) (newValue / skillType.getXpModifier())
				* LoadProperties.xpGainMultiplier;
		...
		Bukkit.getPluginManager().callEvent(
				new McMMOPlayerXpGainEvent(player, skillType, xp));
	 ...
	 * 
	 * @param skillType
	 *            Type of skill to modify
	 * @param newValue
	 *            New level value for the skill
	 */
	 ...
	 * 
	 * @param skillType
	 *            Type of skill to check
	 * @return the XP remaining until next level
	 */
	public int getXpToLevel(SkillType skillType) {
		return (int) (1020 + (skills.get(skillType) * 20)); // Do we REALLY need
															// to cast to int
															// here?
	}
	...
	private double partyModifier(SkillType skillType) {
		...
		for (Player x : Party.getInstance().getPartyMembers(player)) {
			if (x.isOnline()
					&& !x.getName().equals(player.getName())
					&& Party.getInstance().isPartyLeader(x.getName(),
							this.getParty())) {
				if (m.isNear(player.getLocation(), x.getLocation(), 25.0)) {
					PlayerProfile PartyLeader = Users.getProfile(x);

					if (PartyLeader.getSkillLevel(skillType) >= this
							.getSkillLevel(skillType)) {

						int leaderLevel = PartyLeader.getSkillLevel(skillType);
						int difference = leaderLevel
								- this.getSkillLevel(skillType);
